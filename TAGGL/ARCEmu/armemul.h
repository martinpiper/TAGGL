...
class arm_emulator 
{
  ...
  public:
	...
	word_t get_pc() const
	{
		return my_regs.gpr[PC_REAL_IND];
	}

	word_t read_gpr(int index) const
	{
		// MPi: Force 26 bit ARM PSR reads
		if ( (index == PC_AHEAD_IND) || (index == PC_REAL_IND) )
		{
			return my_regs.gpr[index] | (read_cpsr() & 0xc0000003);
		}
		return my_regs.gpr[index];
	}

	word_t read_gprHint(int index, int destiationHint) const
	{
		word_t toRet = my_regs.gpr[index];

		// MPi: Force 26 bit ARM PSR reads
		if ( (index == PC_AHEAD_IND) || (index == PC_REAL_IND) )
		{
			toRet = my_regs.gpr[index] | (read_cpsr() & 0xc0000003);
		}

		if ((destiationHint < 15) && (index >= 15))
		{
			// If the destination is not the PC and the source is the PC then remove the PC flags
			toRet &= 0x3ffffffc;
		}
	...
	{
		int realIndex = index==15?PC_REAL_IND:index;
		write_gpr2(realIndex,val);
	}
