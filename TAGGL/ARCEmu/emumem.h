...
const unsigned int MEMORY_PAGE_SIZE_BITS  = 15;	// MPi: Tweaked this to allow the block below 0x8000 to have different access to the blocks above
...
class memory
{
	public:
		...
		*******************************************************************/
		...
		{
			addr = addr & 0x3fffffff;	// MPi: Force 26bit memory access
		...
		{
			addr = addr & 0x3ffffffc;	// MPi: Force 26bit memory access
		...
		{
			addr = addr & 0x3ffffffe;	// MPi: Force 26bit memory access
		...
		*******************************************************************/
		...
		memory_fault_t read_block(void *buf, target_addr_t addr,
				unsigned int size);
		memory_fault_t write_block(const void *buf, target_addr_t addr,
				unsigned int size);
	...
	private:
		...
		{
			return (pte->flag & MEMORY_PAGE_REMAPPED)?true:false;
		}
		...
		bool page_remapped(target_addr_t addr) const 
		{
			...
			target_addr_t page_index = addr >> MEMORY_PAGE_SIZE_BITS;
			return (page_table[page_index].flag & MEMORY_PAGE_REMAPPED)?true:false;
		}
		...
		/* slow but full featured memory access interface
