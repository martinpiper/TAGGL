...
memory_fault_t memory::write_block(const void *buf, target_addr_t addr,
		unsigned int size)
{
	addr = addr & 0x3fffffff;	// MPi: Force 26bit memory access
	...
	while (len < size)
	{
		if ((ft = write_block_within_page((void *) buf, addr+len, size-len, &sz)) !=
			MEM_NO_FAULT)
			return ft;

		len += sz;
		buf = reinterpret_cast<const byte_t *>(buf) + sz;
	}
	return MEM_NO_FAULT;
}
